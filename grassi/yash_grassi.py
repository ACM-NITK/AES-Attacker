# S-box
sbox = []

Rcon = []

fixed_matrix = [[0x02, 0x03, 0x01, 0x01],[0x01, 0x02, 0x03, 0x01],[0x01, 0x01, 0x02, 0x03],[0x03, 0x01, 0x01, 0x02]]

def get_roundkeys(x):
  #first round key
    keys = [[] for _ in range(44)]
    
    for i in range(16):
          byte = (x >> ( 8 * (15- i ))) & int(0xff)
          keys[int(i / 4)].append(byte)
    #print(keys)
  #rest of the round keys
    for i in range(4,44):
        temp=keys[i-1]
        if i%4==0:
            rot=[]
            rot.append(keys[i-1][1])
            rot.append(keys[i-1][2])
            rot.append(keys[i-1][3])
            rot.append(keys[i-1][0])
            for j in range(4):
              if j==0:
                  byte=sbox[rot[j]] ^ Rcon[int(i/4)]
              else:
                  byte=sbox[rot[j]]
              keys[i].append(byte ^ keys[i-4][j])
        else:
            for j in range(4):
                keys[i].append(keys[i-4][j]^keys[i-1][j])
    #print(keys)    
    return keys

def key_to_state(key_no,keys):
        key = [[] for _ in range(4)]
        for i in range(4):
            for j in range(4):
                key[i].append(keys[key_no+j][i])
        return key

def text_to_state(text):
    matrix = [[] for _ in range(4)]
    
    for i in range(16):
          byte = (text >> ( 8 * (15- i ))) & 0xff
          matrix[int(i / 4)].append(byte)
    matrix2 = [[] for _ in range(4)]
    for i in range(4):
        for j in range(4):
            matrix2[i].append(matrix[j][i])      
    
    return matrix2

def state_to_text(matrix):
    text = 0
    for i in range(4):
        for j in range(4):
            text |= (matrix[j][i] << (120 - 8 * (4 * i + j)))
    #print(text)
    return text

def encrypt(key,matrix):
    keys = get_roundkeys(key)
    
    #adding round 0 key
    cipher = [[[] for __ in range(4) ] for _ in range(11)]
    key0 = key_to_state(0,keys)
    
    for i in range(4):
        for j in range(4):
            cipher[0][i].append(key0[i][j] ^ matrix[i][j])
    
    #rounds 1-4
    for i in range(1,5):
      
        #substitution
        sbox_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                sbox_state[j].append(sbox[cipher[i-1][j][k]])
        #print(sbox_state)
        
        #shift rows
        shiftrow_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                shiftrow_state[j].append(sbox_state[j][(k+j)%4])
        #print(shiftrow_state)
        
        #mix columns
        mixcol_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                sum=0
                for l in range(4):
                    if fixed_matrix[j][l] == 0x01:
                        sum = sum ^ shiftrow_state[l][k]
                    elif fixed_matrix[j][l] == 0x02:
                        if shiftrow_state[l][k]>127:
                            sum = sum ^ (((shiftrow_state[l][k]<<1)-256)^0x1b)
                        else:
                            sum = sum ^ ((shiftrow_state[l][k]<<1))
                    elif fixed_matrix[j][l] == 0x03:
                        if shiftrow_state[l][k]>127:
                            sum = sum ^ ((((shiftrow_state[l][k]<<1)-256)^0x1b)  ^ shiftrow_state[l][k])
                        else:
                            sum = sum ^ (((shiftrow_state[l][k]<<1)) ^ shiftrow_state[l][k])
                    #print(sum)
                mixcol_state[j].append(sum)
        #print(mixcol_state)
                
        #add roundkey
        roundkey = key_to_state(i*4,keys)  
        for j in range(4):
            for k in range(4):
                cipher[i][j].append(roundkey[j][k] ^ mixcol_state[j][k])
        #print(cipher[i])
        
    #round 5 
    
    #substitution
        
    sbox_state = [[] for _ in range(4)]
    for j in range(4):
        for k in range(4):
            sbox_state[j].append(sbox[cipher[4][j][k]])
    #print(sbox_state)

    #shift rows
    shiftrow_state = [[] for _ in range(4)]
    for j in range(4):
        for k in range(4):
            shiftrow_state[j].append(sbox_state[j][(k+j)%4])
    #print(shiftrow_state)
    
    #add roundkey
    roundkey = key_to_state(10*4,keys)  
    for j in range(4):
        for k in range(4):
            cipher[5][j].append(roundkey[j][k] ^ shiftrow_state[j][k])
    return cipher[5]


def encrypt1(key,matrix):
    keys = get_roundkeys(key)
    
    #adding round 0 key
    cipher = [[[] for __ in range(4) ] for _ in range(11)]
    key0 = key_to_state(0,keys)
    
    for i in range(4):
        for j in range(4):
            cipher[0][i].append(key0[i][j] ^ matrix[i][j])
    
    #round 1
    for i in range(1,2):
        #substitution
        sbox_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                sbox_state[j].append(sbox[cipher[i-1][j][k]])
        
        #shift rows
        shiftrow_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                shiftrow_state[j].append(sbox_state[j][(k+j)%4])
        
        #mix columns
        mixcol_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                sum=0
                for l in range(4):
                    if fixed_matrix[j][l] == 0x01:
                        sum = sum ^ shiftrow_state[l][k]
                    elif fixed_matrix[j][l] == 0x02:
                        if shiftrow_state[l][k]>127:
                            sum = sum ^ (((shiftrow_state[l][k]<<1)-256)^0x1b)
                        else:
                            sum = sum ^ ((shiftrow_state[l][k]<<1))
                    elif fixed_matrix[j][l] == 0x03:
                        if shiftrow_state[l][k]>127:
                            sum = sum ^ ((((shiftrow_state[l][k]<<1)-256)^0x1b)  ^ shiftrow_state[l][k])
                        else:
                            sum = sum ^ (((shiftrow_state[l][k]<<1)) ^ shiftrow_state[l][k])
                mixcol_state[j].append(sum)
                
        #add roundkey
        roundkey = key_to_state(i*4,keys)  
        for j in range(4):
            for k in range(4):
                cipher[i][j].append(roundkey[j][k] ^ mixcol_state[j][k])
    return cipher[1]

def encrypt2(key,matrix):
    keys = get_roundkeys(key)
    
    #adding round 0 key
    cipher = [[[] for __ in range(4) ] for _ in range(11)]
    key0 = key_to_state(0,keys)
    
    for i in range(4):
        for j in range(4):
            cipher[0][i].append(key0[i][j] ^ matrix[i][j])
    for i in range(4):
        for j in range(4):
            cipher[1][i].append(key0[i][j] ^ matrix[i][j])
    #round 1
    for i in range(2,5):
        #substitution
        sbox_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                sbox_state[j].append(sbox[cipher[i-1][j][k]])
        
        #shift rows
        shiftrow_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                shiftrow_state[j].append(sbox_state[j][(k+j)%4])
        
        #mix columns
        mixcol_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                sum=0
                for l in range(4):
                    if fixed_matrix[j][l] == 0x01:
                        sum = sum ^ shiftrow_state[l][k]
                    elif fixed_matrix[j][l] == 0x02:
                        if shiftrow_state[l][k]>127:
                            sum = sum ^ (((shiftrow_state[l][k]<<1)-256)^0x1b)
                        else:
                            sum = sum ^ ((shiftrow_state[l][k]<<1))
                    elif fixed_matrix[j][l] == 0x03:
                        if shiftrow_state[l][k]>127:
                            sum = sum ^ ((((shiftrow_state[l][k]<<1)-256)^0x1b)  ^ shiftrow_state[l][k])
                        else:
                            sum = sum ^ (((shiftrow_state[l][k]<<1)) ^ shiftrow_state[l][k])
                mixcol_state[j].append(sum)
                
        #add roundkey
        roundkey = key_to_state(i*4,keys)  
        for j in range(4):
            for k in range(4):
                cipher[i][j].append(roundkey[j][k] ^ mixcol_state[j][k])
      #round 5 
    
    #substitution
        
    sbox_state = [[] for _ in range(4)]
    for j in range(4):
        for k in range(4):
            sbox_state[j].append(sbox[cipher[4][j][k]])

    #shift rows
    shiftrow_state = [[] for _ in range(4)]
    for j in range(4):
        for k in range(4):
            shiftrow_state[j].append(sbox_state[j][(k+j)%4])
    #add roundkey
    roundkey = key_to_state(10*4,keys)  
    for j in range(4):
        for k in range(4):
            cipher[5][j].append(roundkey[j][k] ^ shiftrow_state[j][k])
    return cipher[5]

#for finding the good pair
def good_pair(key):
    plain_1 = [[0,0,0,0] for _ in range(4)]
    plain_2 = [[0,0,0,0] for _ in range(4)]
    for w1 in range(256):
        for x1 in range(256):
            for y1 in range(256):
                for z1 in range(256):
                    for w2 in range (256):
                        for x2 in range (256):
                            for y2 in range (256):
                                for z2 in range (256):
                                    plain_1[0][0]=z1
                                    plain_1[1][1]=y1
                                    plain_1[2][2]=x1
                                    plain_1[3][3]=w1
                                    plain_2[0][0]=z2
                                    plain_2[1][1]=y2
                                    plain_2[2][2]=x2
                                    plain_2[3][3]=w2
                                    plain_enc1=encrypt(key,plain_1)
                                    plain_enc2=encrypt(key,plain_2)
                                    if(plain_enc1[0][0]==plain_enc2[0][0] and plain_enc1[1][1]==plain_enc2[1][1] and plain_enc1[2][2]==plain_enc2[2][2] and plain_enc1[3][3]==plain_enc2[0][0] and (plain_enc1!=plain_enc2)):
                                        return (plain_enc1,plain_enc2)


key = "2b7e151628aed2a6abf7158809cf4f3c"
p1, p2 = good_pair(key)
#guess_key is used for guessing the key
guess_key = [[0,0,0,0] for _ in range(4)]   
print(p1,p2)
for i in range(256):
     for j in range(256):
         for k in range(256):
             for l in range(256):
                #diagonal elements are stored into the guess_key
                guess_key[0][0]=l
                guess_key[1][1]=k 
                guess_key[2][2]=j
                guess_key[3][3]=i
                #encryption  
                #diagonal elements are now in row 1
                xx=encrypt1(guess_key,p1)
                yy=encrypt1(guess_key,p2)
                #copying the elements into matrices w and y
                ww=yy
                zz=xx
                #element 1,0 and 3,0 are interchanged 
                ww[1][0]=xx[1][0]
                ww[3][0]=xx[3][0]
                zz[1][0]=yy[1][0]
                zz[3][0]=yy[3][0]
                #p_final variables store the values after encryption
                p_final1=encrypt2(guess_key,zz)
                p_final2=encrypt2(guess_key,ww)
                #comparing the elements in the two
                #if they match then its a success else we continue looking 
                if(p_final1[0][0]==p_final2[0][0] and p_final2[1][3]==p_final1[1][3] and p_final1[2][2]==p_final2[2][2] and p_final1[3][1]==p_final2[3][1]):
                    print("Task Complete")
                    print(guess_key)
                    







