# -*- coding: utf-8 -*-
"""Hardik_grassi.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ufYBGZcjlSfuTFRr0AYDZ-iqOZOD8mlT
"""

# S-box
sbox = []

Rcon = []

fixed_matrix = [[0x02, 0x03, 0x01, 0x01],[0x01, 0x02, 0x03, 0x01],[0x01, 0x01, 0x02, 0x03],[0x03, 0x01, 0x01, 0x02]]

def get_roundkeys(x):
  #first round key
    keys = [[] for _ in range(44)]
    
    for i in range(16):
          byte = (x >> ( 8 * (15- i ))) & int(0xff)
          keys[int(i / 4)].append(byte)
    
  #rest of the round keys
    for i in range(4,44):
        temp=keys[i-1]
        if i%4==0:
            rot=[]
            rot.append(keys[i-1][1])
            rot.append(keys[i-1][2])
            rot.append(keys[i-1][3])
            rot.append(keys[i-1][0])
            for j in range(4):
              if j==0:
                  byte=sbox[rot[j]] ^ Rcon[int(i/4)]
              else:
                  byte=sbox[rot[j]]
              keys[i].append(byte ^ keys[i-4][j])
        else:
            for j in range(4):
                keys[i].append(keys[i-4][j]^keys[i-1][j])
    
    return keys

def key_to_state(key_no,keys):
        key = [[] for _ in range(4)]
        for i in range(4):
            for j in range(4):
                key[i].append(keys[key_no+j][i])
        
        return key
      
#first round of encryption
def encryptfirstround(key,matrix):
    keys = get_roundkeys(key)
    
    #adding round 0 key
    cipher = [[[] for __ in range(4) ] for _ in range(11)]
    key0 = key_to_state(0,keys)
    
    
    for i in range(4):
        for j in range(4):
            cipher[0][i].append(key0[i][j] ^ matrix[i][j])
    
    
      
        #substitution
        
    sbox_state = [[] for _ in range(4)]
    for j in range(4):
       for k in range(4):
          sbox_state[j].append(sbox[cipher[0][j][k]])
    #print(sbox_state)
        
      #shift rows
    shiftrow_state = [[] for _ in range(4)]
    for j in range(4):
      for k in range(4):
        shiftrow_state[j].append(sbox_state[j][(k+j)%4])
        #print(shiftrow_state)
        
        #mix columns
    mixcol_state = [[] for _ in range(4)]
    for j in range(4):
      for k in range(4):
        sum=0
        for l in range(4):
          if fixed_matrix[j][l] == 0x01:
            sum = sum ^ shiftrow_state[l][k]
          elif fixed_matrix[j][l] == 0x02:
            if shiftrow_state[l][k]>127:
              sum = sum ^ (((shiftrow_state[l][k]<<1)-256)^0x1b)
            else:
              sum = sum ^ ((shiftrow_state[l][k]<<1))
          elif fixed_matrix[j][l] == 0x03:
            if shiftrow_state[l][k]>127:
              sum = sum ^ ((((shiftrow_state[l][k]<<1)-256)^0x1b)  ^ shiftrow_state[l][k])
            else:
              sum = sum ^ (((shiftrow_state[l][k]<<1)) ^ shiftrow_state[l][k])
                    
    mixcol_state[j].append(sum)
        #print(mixcol_state)
                
        #add roundkey
    roundkey = key_to_state(i*4,keys)  
    for j in range(4):
      for k in range(4):
        cipher[1][j].append(roundkey[j][k] ^ mixcol_state[j][k])
        #print(cipher[1])
        
    
    return cipher[1]

#remaining rounds of encryption
def encrypt4rounds(key,matrix):
    keys = get_roundkeys(key)
    
    
    cipher = [[[] for __ in range(4) ] for _ in range(11)]
    key0 = key_to_state(0,keys)
    cipher[0] = matrix
    cipher[1] = matrix
    
    #rounds 2-4
    for i in range(2,5):
      
        #substitution
        
        sbox_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                sbox_state[j].append(sbox[cipher[i-1][j][k]])
        #print(sbox_state)
        
        #shift rows
        shiftrow_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                shiftrow_state[j].append(sbox_state[j][(k+j)%4])
        #print(shiftrow_state)
        
        #mix columns
        mixcol_state = [[] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                sum=0
                for l in range(4):
                    if fixed_matrix[j][l] == 0x01:
                        sum = sum ^ shiftrow_state[l][k]
                    elif fixed_matrix[j][l] == 0x02:
                        if shiftrow_state[l][k]>127:
                            sum = sum ^ (((shiftrow_state[l][k]<<1)-256)^0x1b)
                        else:
                            sum = sum ^ ((shiftrow_state[l][k]<<1))
                    elif fixed_matrix[j][l] == 0x03:
                        if shiftrow_state[l][k]>127:
                            sum = sum ^ ((((shiftrow_state[l][k]<<1)-256)^0x1b)  ^ shiftrow_state[l][k])
                        else:
                            sum = sum ^ (((shiftrow_state[l][k]<<1)) ^ shiftrow_state[l][k])
                    #print(sum)
                mixcol_state[j].append(sum)
        #print(mixcol_state)
                
        #add roundkey
        roundkey = key_to_state(i*4,keys)  
        for j in range(4):
            for k in range(4):
                cipher[i][j].append(roundkey[j][k] ^ mixcol_state[j][k])
        #print(cipher[4])
        
    #round 5 
    
    #substitution
        
    sbox_state = [[] for _ in range(4)]
    for j in range(4):
        for k in range(4):
            sbox_state[j].append(sbox[cipher[4][j][k]])
    #print(sbox_state)

    #shift rows
    shiftrow_state = [[] for _ in range(4)]
    for j in range(4):
        for k in range(4):
            shiftrow_state[j].append(sbox_state[j][(k+j)%4])
    #print(shiftrow_state)
    
    #add roundkey
    roundkey = key_to_state(10*4,keys)  
    for j in range(4):
        for k in range(4):
            cipher[5][j].append(roundkey[j][k] ^ shiftrow_state[j][k])
    #print(cipher[5])
    return cipher[5]

  
  
key = 0x2b7e151628aed2a6abf7158809cf4f3c
print("key :", end =" "),
print(key)
print("\n")

#Finding good pair
plain_text = [[0,0,0,0] for _ in range(4)]
plain_text2 = [[0,0,0,0] for _ in range(4)]
for i in range(256):
    for j in range(256):
        for k in range(256):
            for l in range(256):
                plain_text2[0][0]=l
                plain_text2[1][1]=k
                plain_text2[2][2]=j
                plain_text2[3][3]=i
                for i1 in range(256):
                    for j1 in range(256):
                        for k1 in range(256):
                            print(i,j,k)
                            for l1 in range(256):
                                plain_text[0][0]=l1
                                plain_text[1][1]=k1
                                plain_text[2][2]=j1
                                plain_text[3][3]=i1
                                p1 = encrypt4rounds(key,encryptfirstround(key,plain_text2)) #encryption of the plaintext
                                p2 = encrypt4rounds(key,encryptfirstround(key,plain_text))
                                if(p2[0][0]==p1[0][0] and p2[1][3]==p1[1][3] and p2[2][2]==p1[2][2] and p2[3][1]==p1[3][1] and p2!=p1):
                                    print("Good pair found")
                                    gp1=plain_text2
                                    gp2=plain_text
                                    i=255
                                    i1=255
                                    j=255
                                    j1=255
                                    k=255
                                    k1=255
                                    l=255
                                    l1=255
                                

#good pair of plain text are found
print(gp1,gp2)
key_try = [[0,0,0,0] for _ in range(4)]        
for i in range(256):
    for j in range(256):
        for k in range(256):
            for l in range(256): 
                key_try[0][0]=l
                key_try[1][1]=k
                key_try[2][2]=j
                key_try[3][3]=i
                xp=encryptfirstround(key_try,gp1)	#partial encryption of good pair
                yp=encryptfirstround(key_try,gp2)
                zp=xp
                wp=yp
                #creating a mixture of the encrypted pair
                zp[1][0]=yp[1][0]
                zp[3][0]=yp[3][0]
                wp[1][0]=xp[1][0]
                wp[3][0]=xp[3][0]
                p3=encrypt4rounds(key_try,zp)#performing the remaining rounds of encryption on the mixture
                p4=encrypt4rounds(key_try,wp)
                if(p3[0][0]==p4[0][0] and p3[1][3]==p4[1][3] and p3[2][2]==p4[2][2] and p3[3][1]==p4[3][1]):
                  print(key_try)
                  print("key obtained")